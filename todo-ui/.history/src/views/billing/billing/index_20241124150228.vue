<template>
  <div class="app-container">
    ...
    <el-date-picker clearable v-model="time" type="datetimerange" value-format="YYYY-MM-DD HH:mm:ss" range-separator="至" start-placeholder="开始日期时间" end-placeholder="结束日期时间" @change="handleTimeChange" />
  </el-form-item>
...
watch(time, (newVal) => {
  if (newVal && newVal.length === 2) {
    queryParams.beginTime = newVal[0];
...
function handleTimeChange(newVal) {
  if (newVal && newVal.length === 2) {
    queryParams.beginTime = newVal[0];
    queryParams.endTime = newVal[1];
  } else {
