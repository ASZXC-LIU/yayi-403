<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <span>当天单数：</span><div class="stat-value">{{ todayBillingCount }}</div>
      <span>总金额</span><div class="stat-value">{{ todayTotalAmount }}</div>
      <span>已支付</span><div class="stat-value">{{ todayPaidAmount }}</div>
      <span>未支付</span><div class="stat-value">{{ todayUnpaidAmount }}</div>
      <!-- 生成一个当天的单数，总金额，已支付金额，以及未支付金额 -->
    </el-form>
...
import {useDateStore}from'@/store/modules/useDateStore';
...
function getTodayStats() {
  listBilling({
    billingDate: new Date().toISOString().split("T")[0], // 仅查询当天数据
  }).then((response) => {
    const billingList = response.rows;
    todayBillingCount.value = billingList.length;
    todayTotalAmount.value = billingList.reduce((sum, item) => sum + item.totalAmount, 0);
    todayPaidAmount.value = billingList.reduce((sum, item) => item.paymentStatus === '已支付' ? sum + item.totalAmount : sum, 0);
    todayUnpaidAmount.value = todayTotalAmount.value - todayPaidAmount.value;
  });
}
console.log(dataStore.date);
// 页面加载时获取统计数据
getTodayStats();
