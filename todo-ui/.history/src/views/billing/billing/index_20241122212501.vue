...
function getTodayStats() {
  ...
  listBilling({}).then((response) => {
    ...
    // 统计账单数量
    todayBillingCount.value = todayBillings.length;

    // 统计总金额（保留两位小数）
    todayTotalAmount.value = formatAmount(
      todayBillings.reduce((sum, item) => sum + item.totalAmount, 0)
    );

    // 统计已支付金额（paymentStatus = 1，保留两位小数）
    todayPaidAmount.value = formatAmount(
      todayBillings
        .filter((item) => item.paymentStatus === "1")
        .reduce((sum, item) => sum + item.totalAmount, 0)
    );

    // 统计未支付金额（paymentStatus = 2，保留两位小数）
    todayUnpaidAmount.value = formatAmount(
      todayBillings
